...
export const CharacterAvatar = ({ character, size = 'medium' }: CharacterAvatarProps) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (groupRef.current) {
        if (size === 'mini') {
          ...
          // Automatic idle swing around the base rotation
          if (!isDragging.current) {
            groupRef.current.rotation.y = rotationY.current + Math.sin(elapsed * 0.5) * 0.4;
          }
      ...
      // Check if halo exists before rotating
      const halo = scene.children.find(child => child.type === 'Mesh' && (child as any).geometry?.type === 'TorusGeometry');
      if (halo) {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      if (size === 'mini' || !groupRef.current) return;
      isDragging.current = true;
      // Capture current total rotation to avoid jumping when we stop the sin wave
    ...
    const handleMouseUp = () => {
      ...
      // Adjust anchor so resumption of sin wave is perfectly smooth from current position
      const elapsed = clock.getElapsedTime();
      rotationY.current = groupRef.current.rotation.y - Math.sin(elapsed * 0.5) * 0.4;
    };
  ...
  }, [dimensions, character.head, character.shirt, character.pants, character.feet, size]);
  ...
  return (
    <div
      ...
      className={`
        border-2 border-cyan-500/50 shadow-lg shadow-cyan-500/20 overflow-hidden 
        bg-[radial-gradient(circle_at_50%_25%,rgba(34,211,238,0.22),rgba(15,23,42,0.96)_68%)]
        ${size === 'mini' ? 'rounded-full' : 'rounded-2xl'}
        ${size !== 'mini' ? 'cursor-grab active:cursor-grabbing' : ''}
      `}
    />
  );
};
