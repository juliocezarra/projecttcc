...
function createHeadAccessory(itemId: string): THREE.Group {
  ...
  switch (itemId) {
    case 'head-1': {
      ...
      leftGlass.position.set(-0.2, 0, 0.46);
      group.add(leftGlass);
      const rightGlass = new THREE.Mesh(glassGeo, glassMat);
      rightGlass.position.set(0.2, 0, 0.46);
...
function createShirt(itemId: string): THREE.Group {
  ...
  switch (itemId) {
    ...
    case 'shirt-2': {
      ...
      const leftSleeve = new THREE.Mesh(sleeveGeo, mat);
      leftSleeve.position.set(-0.55, 0.1, 0);
      leftSleeve.rotation.z = 0.3;
      group.add(leftSleeve);
      const rightSleeve = new THREE.Mesh(sleeveGeo, mat);
      rightSleeve.position.set(0.55, 0.1, 0);
    ...
    case 'shirt-5': {
      ...
      const leftSleeve = new THREE.Mesh(sleeveGeo, mat);
      leftSleeve.position.set(-0.55, 0.05, 0);
      leftSleeve.rotation.z = 0.3;
      group.add(leftSleeve);
      const rightSleeve = new THREE.Mesh(sleeveGeo, mat);
      rightSleeve.position.set(0.55, 0.05, 0);
      rightSleeve.rotation.z = -0.3;
      group.add(rightSleeve);
...
export const CharacterAvatar = ({ character, size = 'medium' }: CharacterAvatarProps) => {
  ...
  useEffect(() => {
    ...
    mouth.position.set(0, 1.68, 0.45);
    group.add(mouth);

    // Neck
    const neckGeo = new THREE.CylinderGeometry(0.12, 0.18, 0.25, 16);
    const neck = new THREE.Mesh(neckGeo, headMat);
    neck.position.y = 1.35;
    group.add(neck);

    // Arms (visible skin, no spheres at ends)
    const armGeo = new THREE.CylinderGeometry(0.08, 0.07, 0.95, 12);
